...
export default function ProjectCard({ image, title, description, link, tag } : Props) {
  return(
    <div className="max-w-[450px] h-full md:h-auto flex-shrink-0 bg-dark-1 text-neutral-1 rounded-[16px] relative select-none">
      ...
      <div className="px-7 pt-5 pb-[55px]">
        <h3 className="text-[24px] font-semibold leading-[100%] mb-2">{title}</h3>
        <p className="text-[12px] max-w-[335px]">{description}</p>
        <Image src={ProjectDeco} alt="" className="absolute bottom-0 right-0 w-[120px] md:w-[225px]" />
      </div>
